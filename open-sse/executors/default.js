...
const REFRESH_GRANTS = Object.fromEntries(
  Object.entries(PROVIDER_OAUTH)
    .filter(([, o]) => o.refresh)
    .map(([id, o]) => {
      // Prefer refreshUrl when available (autoclaw has separate refresh endpoint);
      // fall back to tokenUrl for standard OAuth providers.
      const refreshUrl = o.refreshUrl || o.tokenUrl;
