 ...
 * can be mapped back for the client. See chatCore.js (request wiring) and
 * translator/response/kiro-to-openai.js (response reverse-mapping).
 */

import { createHash } from "node:crypto";
import { KIRO_TOOL_NAME_MAX_LENGTH } from "../config/kiroConstants.js";
...
export function sanitizeKiroTools(tools) {
  ...
  const sanitized = tools.map((tool) => {
    ...
    if (typeof name === "string" && name.length > KIRO_TOOL_NAME_MAX_LENGTH) {
