...
const AUTOCLAW_PROXY_TIMEOUT_MS = 300000; // 5 minutes
const AUTOCLAW_PROXY_PORT = 18432;
const AUTOCLAW_CALLBACK_PATH = "/auth/callback-google";
...
export function startAutoClawProxy() {
  return new Promise((resolve) => {
    ...
    const server = http.createServer(async (req, res) => {
      ...
      if (url.pathname !== AUTOCLAW_CALLBACK_PATH) {
        res.writeHead(404);
        res.end("Not found");
        return;
      }
    ...
    server.listen(AUTOCLAW_PROXY_PORT, "127.0.0.1", () => {
      autoclawProxyServer = server;
      autoclawProxyTimeout = setTimeout(
        () => stopAutoClawProxy(),
        AUTOCLAW_PROXY_TIMEOUT_MS,
      );
      console.log(
        `[autoclaw-proxy] listening on 127.0.0.1:${AUTOCLAW_PROXY_PORT}${AUTOCLAW_CALLBACK_PATH}`,
      );
      resolve({ success: true });
    });
